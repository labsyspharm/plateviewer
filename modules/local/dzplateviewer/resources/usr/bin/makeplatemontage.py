...
class ImageCreator(object):
    ...
    def create(self, source, destination, cores):
        ...
        with Parallel(n_jobs=cores,backend="threading") as parallel:
...
def parseArguments():
    ...
    parser.add_argument(
        '--pattern',
        help='Pattern to define image filenames.',
        type=str,
        default='{well}f{field:02}d{channel}.tif',
    )
    ...
    parser.add_argument(
        '-f',
        '--outfile',
        help='Name of the output DZI file.',
        type=str,
        default='dzi')
    ...
    parser.add_argument(
        '-o',
        '--outdir',
        help='Output folder to put DZI tiling.',
        type=str,
        default='dzi')
    ...
    parser.add_argument(
        '-p',
        '--platedim',
        help='Plate dimensions. Provide two integers separated by a white space.',
        nargs=2,
        type=int,
        default=(24, 16))
    ...
    parser.add_argument(
        '-w',
        '--welldim',
        help='Well dimensions. Provide two integers separated by a white space.',
        nargs=2,
        type=int,
        default=(4, 4))
    ...
    parser.add_argument(
        '-m',
        '--imdim',
        help='Image dimensions. Provide two integers separated by a white space.',
        nargs=2,
        type=int,
        default=(1104, 1104))
    ...
    parser.add_argument(
        '-I',
        '--imint',
        help='Image intensities for rescaling. Provide two integers separated by a white space.',
        nargs=2,
        type=int,
        default=(250, 3000))
    ...
    parser.add_argument(
        '-c',
        '--imch',
        help='Channel of the image to process.',
        type=int,
        default=0)
    ...
    parser.add_argument(
        '-q',
        '--imquality',
        help='Image quality (0.1 - 1) for JPG or compression level for PNG.',
        type=float,
        default=0.8)
...
def processWell(inRow, inCol, pattern, dry_run=False):
    ...
    locIrow = inRow
    locIcol = inCol
    locIwell = f'{locIrow}{locIcol:02}'

    # Read images of FOVs of a single well

    for locIfov in wellFOVs:

        # eval is used here so that the f-string pattern specified on the
        # command line can have arbitrary expressions to allow more
        # flexibility. It would be a security risk (ACE) if this script were to
        # be exposed to outside users, but for a local user on the command line
        # there's no real problem.
        locImName = eval('f"""' + pattern + '"""', dict(well=locIwell, field=locIfov, channel=imCh))
        if '?' in locImName or '*' in locImName:
            matches = [str(p) for p in imDir.glob(locImName)]
            if len(matches) == 1:
                locImPath = matches[0]
            if len(matches) == 0:
                locImPath = locImName  # Just to make debug logging helpful
                if dry_run:
                    print(f'MISSING: {locImName}')
                    continue
            elif len(matches) > 1:
                locImPath = locImName  # Just to make debug logging helpful
                print(f'\nToo many matches for "{locImName}":')
                for i, m in enumerate(matches):
                    print(f'  {i}: {m}')
                if dry_run:
                    continue
        else:
            locImPath = f'{imDir}/{locImName}'
        if(DEB):
            print("\nChecking:", locImPath)
        ...
        if dry_run:
            if flagFileExists:
                print(f'OK     : {locImPath}')
            else:
                print(f'MISSING: {locImPath}')
            continue
...
if __name__ == "__main__":
    ...
    for iRow in range(0, plateHeight):
        for iCol in plateCol:
            ...
            imWell = processWell(plateRow[iRow], iCol+1, args.pattern, args.dry_run)
            if not args.dry_run:
